 ...
 */
object SparkMovieRecommendation {
  def main(args: Array[String]) {
    ...
    val ratings = sc.textFile(new File(movieLensHomeDir, "music_ratings.dat").toString).map { line =>
      val fields = line.split("::")
      // format: (timestamp % 10, Rating(userId, movieId, rating))
      (fields(3).toLong % 10, Rating(fields(0).toInt, fields(1).toInt, fields(2).toDouble))
    }

    val movies = sc.textFile(new File(movieLensHomeDir, "musicCategories.dat").toString).map { line =>
      val fields = line.split("::")
