...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("music","movie","sport","food","uselections")
  ...
  def classifyImage(sc: SparkContext, path: String): Unit = {
    ...
    val p = nbModel.predict(histogram)
    println("The predicted image category is  : " + IMAGE_CATEGORIES(p.toInt))
  ...
  def main(args: Array[String]) {
    ...
    val ip = InetAddress.getByName("127.0.1.1").getHostName
    ...
    val base64Strings = data.filter(line => {
